...
## Title: GUI functions                                                         ##
##                                                                              ##
## What: All functions related to the GUI and PySide2.                          ##
##                                                                              ##
##################################################################################
...
from functions.sql_functions import (
    ...
    add_sql_data,
    delete_sql_data,
    db_query_sample_type,
    db_query_group,
    del_sql_entry,
)
from functions.settings_functions import (
    config_check_selected_db,
    config_write_selected_db,
    read_columns,
)
...
def update_db_list(self):
    ...
    for file in os.scandir("databases"):
        ...
        if file.name.endswith(".db") or file.name == ".db":
            ...
            item.setText(
                0, QCoreApplication.translate("MainWindow", file.name[:-3], None)
            )
...
def change_database(self, button):
    ...
    if button == 1:
        ...
        self.ui.tree_select_database_2.setCurrentItem(
            self.ui.tree_select_database_2.topLevelItem(index)
        )
        ...
        # Update other table location
        self.ui.tree_select_database.setCurrentItem(
            self.ui.tree_select_database.topLevelItem(index)
        )
...
def load_sql_data(self):
    ...
    # Load data number
    self.ui.label_n_samples.setText(
        str(n_samples(self.ui.tree_select_database.currentItem().text(0)))
    )

    # Write to  QTableWidget
    for i, data in enumerate(sql_data):
        for j, column in enumerate(columns):
            self.ui.tableWidget_db_view.setItem(
                i, j, QtWidgets.QTableWidgetItem(str(data.__dict__[column]))
            )
...
def load_settings(self):
    ...
    try:
        ...
        # Set database text
        self.ui.label_current_database.setText(
            self.ui.tree_select_database.currentItem().text(0)
        )
...
def pie_chart(self):
    ...
    colours = [
        ...
        QColor(197, 67, 62),
        QColor(83, 167, 185),
    ]  #  TODO update to prettier colours
...
def delete_entry_table(self):
    identifier = self.ui.tableWidget_db_view.item(
        self.ui.tableWidget_db_view.currentRow(), 1
    ).text()
    db_name = self.ui.tree_select_database.currentItem().text(0)
    session = engine_connect(db_name)
